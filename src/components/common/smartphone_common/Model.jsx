  ...
  return (
    <div className={`py-[60px] font-markot h-auto flex justify-center ${isLightTheme ? 'bg-white text-black' : 'bg-[#0A0C08] text-white'}`}>
      <div className="w-full lg:content flex flex-col gap-[36px] px-4">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-[12px] md:gap-[24px] justify-center">
          {models.map((model, index) => (
           <div
           key={model.id}
           className={`w-full ${isLightTheme ? 'bg-gray-100 text-black' : 'bg-[#232323] text-white'} rounded-[16px] gap-7 p-4 md:p-6 lg:p-[24px] ${index === 2 ? 'hidden lg:block' : 'block'} flex flex-col justify-between h-full`}
         >
           ...
           <Link to={model.path} className="mt-auto">
             <button
               className={`flex gap-3 justify-center border ${isLightTheme ? 'border-black' : 'border-white/50'} md:py-3 py-1.5 md:px-6 px-1.5 rounded-full items-center w-full text-mobile/small/button md:text-desktop/button text-center uppercase ${isLightTheme ? 'text-black' : 'text-stone-100'}`}
             >
