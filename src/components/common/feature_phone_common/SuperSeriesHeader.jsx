...
export const RoutesData = [
    { label: 'super guru 4g', alias: 'sg 4g', path: '/product/super-guru-4g' },
    { label: 'super guru 400', alias: 'sg 400', path: '/product/super-guru-400' },
    { label: 'super guru 600', alias: 'sg 600', path: '/product/super-guru-600' },
]

const SuperSeriesHeader = ({purchaseLink}) => {

    const { pathname } = useLocation()

    return (
        <div className='px-4 md:px-0 bg-grey/grey/5 font-markot'>
            <div className="content py-3 flex justify-between items-center">
                <div className="flex gap-5 lg:gap-3 items-center">
                    {
                        RoutesData.map(({ label, path, alias }) => (
                            <Link key={path} to={path}>
                                <button className={` ${pathname === path ? 'px-3 lg:px-5 lg:py-[14px] py-2 rounded-full border border-itel-red' : ''}`}>
                                    <span className={`text-mobile/button lg:text-desktop/title ${pathname === path ? 'text-itel-red' : 'text-grey/grey/2'} uppercase`}>
                                        {pathname === path ? label : alias}
                                    </span>
                                </button>
                            </Link>
                        ))
...
export default SuperSeriesHeader
