...
const ColorImageCarousel = ({ ImageCarouselData, showTitleInMobile }) => {
  ...
  return (
    <>
    <div className={`px-4 md:px-0 relative h-full flex flex-col justify-center items-start gap-8 ${ImageCarouselData[0].bgContainer}`}>
        ...
        <div className="lg:rounded-2xl overflow-hidden relative w-full h-full bg-contain lg:h-screen min-h-[360px] md:min-h-full md:bg-contain bg-no-repeat bg-center flex flex-col justify-center items-center bg-a05iImageCarouselMobileBG xs:bg-a05iImageCarouselLaptopBG ">
            

            {/* Phone Image */}
            <img 
              src={activeColorData.images[currentImageIndex]} 
              alt="Phone" 
              className="lg:h-[70%] xs:w-[50%] sm:w-[50%] object-contain" 
            />
            
            <div className="flex flex-row justify-between items-center w-full absolute px-5 md:px-[120px] lg:px-[80px] xl:px-[120px]">
                {/* Left Arrow */}
                <button onClick={handlePrev} className="w-8 h-8 lg:w-12 lg:h-12">
                    <img src="/smartphone/A05i/leftArrowCarouselBlack.svg" alt="leftArrowCarousel"/>
                </button>

                {/* Right Arrow */}
                <button onClick={handleNext} className="w-8 h-8 lg:w-12 lg:h-12">
                        <img src="/smartphone/A05i/rightArrowCarouselBlack.svg" alt="leftArrowCarousel"/>
                </button>
            </div>
        </div>
