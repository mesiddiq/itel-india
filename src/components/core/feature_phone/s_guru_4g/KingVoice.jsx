        ...
        <div className={`flex flex-col overflow-hidden items-center  relative gap-6 lg:gap-14 pb-[366px] pt-9 lg:pb-[116px] lg:pt-12`} style={linearGradient ? { backgroundImage: bgGradient } : { background: bgColor }}>
            <div className="">
                <div className={`flex justify-center border-2 relative rounded-s `} style={{ borderColor: borderColor }}>
                    ...
                    <h1 className='uppercase text-[94px] md:text-[150px] text-center md:leading-[140px] lg:text-[240px] lg:leading-[286px] font-bold leading-[94px] font-markot px-5 lg:px-0 py-4 lg:py-5' style={{ color: textColor }}>
                        <span className='tracking-[15px] font-markot '>king</span><br className='lg:hidden' />
                        <div className='-tracking-[3.7px] inline'>v
                            <div className='relative inline'>
                                o
                                <svg width="92" className='size-6 md:size-12 lg:size-[92px] absolute top-[52%] left-[53%] -translate-x-1/2 -translate-y-1/2' height="92" viewBox="0 0 92 92" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className="grid gap-x-4 gap-y-3 lg:gap-y-10 grid-cols-2 lg:gap-x-[622px] font-markot relative">
                <For each={cards} render={({ label, icon }, idx) =>
                    <div className="gap-2 flex  lg:gap-4 lg:w-[259px] items-center" key={idx} data-aos="fade-up">
                        <div className="lg:p-3 p-[6px] md:p-2 lg:min-w-[54px] lg:rounded-[8px] rounded-[6px] flex items-center justify-center" style={{ background: borderColor }}>
                            <img src={icon} alt={label} className='lg:min-h-8 lg:min-w-8  md:min-h-5 md:min-w-5 min-h-4 min-w-4' />
                        </div>
                        <h3 className='text-mobile/h7 md:text-mobile/small/button lg:text-desktop/h5/medium text-grey/grey/1 max-w-[116px] md:max-w-full'>{label}</h3>
                    </div>
                } />
            </div>
            <img src={phoneUrl} alt="Super Guru 4g" loading='lazy' className='absolute bottom-0 left-1/2 top-[470px] md:top-[580px] lg:top-[278px] -translate-x-1/2 max-w-[298px] max-h-[760px] lg:max-w-[490px] ' />
        </div>
    )
}
