import React from 'react'

const Herosection = () => {
  return (
    <div className='lg:bg-[url("/static_page/promise/days%20promise%20banner%20BG.webp")] flex items-center font-markot  bg-[url("/static_page/promise/days%20promise%20banner%20BG%20mobile.webp")] bg-cover bg-center bg-no-repeat lg:min-h-[520px]  min-h-[450px] bg-url'>
    <div className=" lg:content px-4 ">
      <div className=" lg:w-[918px] text-white w-[280px] space-y-[12px] lg:space-y-5">
        <p className=" text-mobile/h1  lg:text-desktop/h1">
        100 Days Service Promise
        </p>
        <p className="  text-desktop/body/1 lg:text-desktop/body/large">
        No hassle, no stress—just seamless service with our 100-day replacement promise. Should your mobile show any defects within the first 100 days, we'll swap it out for a brand-new one, absolutely free.
        </p>
      </div>
    </div>
  </div>
  )
}

export default Herosection