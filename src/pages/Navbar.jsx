...
const CommonCard = ({ thumbnail, label, price, link, fn = () => { } }) => (
    <Link to={link} onClick={() => fn((prev) => !prev)}>
        <div className="xl:w-[290px] text-white h-[163px] p-6 flex gap-4 bg-dark/card/bg rounded-[8px]">
            <img src={thumbnail || "/placeholder.svg"} alt={label} className="h-[114px] w-[120px] object-contain" />
            <div className="flex flex-col gap-4">
                <h3 className="text-desktop/body/large">{label}</h3>
                <h3 className="text-desktop/button text-left flex items-center">
                    {price === "0" ? (
                        "Coming Soon"
                    ) : (
                        <>
...
// Reusable dropdown component for product categories
const CategoryDropdown = ({ isOpen, data, setIsOpen }) => {
    const [selectedCategory, setSelectedCategory] = useState(data[0])

    const handleClick = (id) => {
        const selCategory = data.find((item) => item.id === id)
        setSelectedCategory(selCategory)
    }

    return (
        <div
            className={`bg-[#1a1a1a] absolute top-0 w-full overflow-hidden transition-all duration-500 ${isOpen ? "h-[330px] py-12 z-[500]" : "h-0"}`}
        >
            <div className="max-w-[1065px] w-full mx-auto flex lg:gap-6 pl-4 xl:pl-0 xl:gap-11">
                <div className="flex flex-col gap-3">
                    {data.map(({ id }) => (
                        <button
                            ...
                            className="flex w-[114px] justify-between gap-2 items-center"
                        >
                            <h3 className="text-desktop/button text-left uppercase text-white">{id}</h3>
                            <ArrowRight isActive={selectedCategory.id === id} />
                        </button>
                    ))}
                    {data === PhoneData && (
                        <Link to="/smartphones">
                            <button className="flex w-[114px] justify-between gap-2 items-center" onClick={() => setIsOpen(false)}>
                                <h3 className="text-desktop/button text-left uppercase text-white">view all</h3>
                                <ArrowRight />
                            </button>
                        </Link>
                    )}
                ...
                <div className="flex flex-col gap-5 w-full">
                    <h3 className="text-desktop/h6/medium text-blue/blue/5">Feature Product</h3>
                    <div className="flex gap-4">
                        {selectedCategory.products.map((item) => (
                            <CommonCard key={item.id} {...item} fn={setIsOpen} />
                        ))}
                    </div>
                </div>
            </div>
...
const Support = ({ support, setSupport }) => (
    <div
        className={`bg-[#1a1a1a] absolute top-0 w-full overflow-hidden transition-all duration-500 ${support ? "h-[330px] z-[500]" : "h-0"}`}
    >
            ...
            {SupportData.map(({ id, label, path }) => (
                <Link to={path} key={id}>
                    <div
                        ...
                        className="py-[7px] flex flex-col justify-center px-6 space-y-4 w-[230px] h-[106px] rounded-[8px] text-white bg-gradient-to-r from-[rgba(255,7,60,0.2)] to-[rgba(55,63,65,0.2)]"
                    >
                        <h3 className="text-white text-desktop/h5">{label}</h3>
...
const Navbar = () => {
    ...
    useEffect(() => {
        if (searchTerm) {
            const filteredPhones = PhonesData.filter((phone) => phone.label.toLowerCase().includes(searchTerm.toLowerCase()))
    ...
    const toggleDropdown = (setter) => {
        handleOutside() // Close all other dropdowns
        setter((prev) => !prev) // Toggle the selected dropdown
    }
    ...
    const navItems = [
        {
            ...
            isDropdown: true,
            isActive: smartPhone,
            onClick: () => toggleDropdown(setSmartPhone),
        },
        {
        ...
        },
        {
            ...
            onClick: () => toggleDropdown(setAccessories),
        },
        {
            label: "Smart TVs",
            isDropdown: true,
            isActive: tv,
            onClick: () => toggleDropdown(setTv),
        },
        {
            label: "Home Care",
            isDropdown: false,
        ...
        },
        {
            label: "About Us",
            isDropdown: false,
            path: "/about-us-itel-mobile-india",
            onClick: handleOutside,
        },
        {
            label: "Support",
            isDropdown: true,
            isActive: support,
            onClick: () => toggleDropdown(setSupport),
        },
    ]
                ...
                <div className="h-[50px] max-w-[1281px] w-11/12 mx-auto flex justify-between items-center">
                                                ...
                                                <div>
                                                    ...
                                                    <ul className="space-y-2">
                                                        {results.smartphones.map((phone) => (
                                                            <li
                                                                onClick={() => navigateToSearch(phone.link)}
                                                                key={phone.id}
                                                                className="text-desktop/h6/medium uppercase group cursor-pointer text-grey/grey/3 py-[6px] flex justify-between items-center"
                                                            >
                    ...
                    <div className="flex gap-2 items-center relative">
                        ...
                        <div className="text-white">
                            <h3 className="text-desktop/overline">itel Customer Care</h3>
                            <a href="tel:1800-4190-525" className="text-desktop/caption">
                                1800-4190-525
                            </a>
                        </div>
                    </div>
            ...
            <div className="font-markot bg-[#1f1f1f] hidden lg:block" ref={navRef}>
                <div className="max-w-[1050px] mx-auto w-11/12 justify-between py-4 flex text-white">
                    {navItems.map((item, index) =>
                        item.isDropdown ? (
                            <button key={index} className="flex items-center gap-0.5" onClick={item.onClick}>
                                <span className="text-desktop/body/2/regular">{item.label}</span>
                                <ArrowSvg isActive={item.isActive} />
                            </button>
                        ) : (
                            <Link key={index} to={item.path}>
                                <button className="flex items-center gap-0.5" onClick={item.onClick}>
                                    <span className="text-desktop/body/2/regular">{item.label}</span>
                                </button>
                            </Link>
                        ),
                ...
                <div className="relative z-[100]">
                    ...
                    <CategoryDropdown isOpen={smartPhone} data={PhoneData} setIsOpen={setSmartPhone} />
                    <CategoryDropdown isOpen={accessories} data={AccessoriesData} setIsOpen={setAccessories} />
                    <CategoryDropdown isOpen={tv} data={TvData} setIsOpen={setTv} />
                </div>
            </div>
        </>
    )
