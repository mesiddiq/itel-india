...
const Compare = () => {
  ...
  const [currentPhoneData, setCurrentPhoneData] = useState(AllFeaturePhoneData[0])
  const [secondPhoneData, setSecondPhoneData] = useState(AllFeaturePhoneData[1])
  const [currentPhone, setCurrentPhone] = useState(AllFeaturePhoneData[0].name)
  const [secondPhone, setSecondPhone] = useState(AllFeaturePhoneData[1].name)


  useEffect(() => {
    setCurrentPhoneData(AllFeaturePhoneData.find((phone => phone.name === currentPhone)))
    setSecondPhoneData(AllFeaturePhoneData.find((phone => phone.name === secondPhone)))
  }, [currentPhone, secondPhone])
            ...
            <div className="flex flex-col gap-[10px]">
              ...
              <div className='flex flex-row justify-between gap-4 overflow-y-scroll overflowHidden'>
                ...
                <ShowDataForComparision
                  ...
                  allPhoneData={AllFeaturePhoneData.filter(phone => phone.name !== secondPhone)}
                  setCurrentPhone={setCurrentPhone}
                />
                <ShowDataForComparision
                  currentPhoneData={secondPhoneData}
                  allPhoneData={AllFeaturePhoneData.filter(phone => phone.name !== currentPhone)}
